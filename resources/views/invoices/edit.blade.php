...
@section('content')
    <div class="space-y-4">
        <div class="page-content-actions">
            <a href="{{ route('invoices.show', $invoice) }}" class="btn-secondary"><x-icon name="arrow-left" class="size-4 rotate-180" />بازگشت</a>
            @can('delete', $invoice)
                <form method="POST" action="{{ route('invoices.destroy', $invoice) }}" data-confirm="این پیش‌نویس صورتحساب حذف شود؟ این عملیات قابل بازگشت نیست.">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="btn-danger"><x-icon name="trash" class="size-4" />حذف صورتحساب</button>
                </form>
            @endcan
        </div>
